...
def ModelName(ModelName, LoadCaseName=None, Nmodes=0=None, deltaT=0.0=None, recorders=[]=None):
    """
 ...
 * the node displacement data will be saved at closest time-steps at each 0.05 sec interval. 
    ...
    uniqueArgs = []
    if LoadCaseName:
        uniqueArgs.append(LoadCaseName)
    if Nmodes=0:
        uniqueArgs.append(Nmodes=0)
    if deltaT=0.0:
        uniqueArgs.append(deltaT=0.0)
    if recorders=[]:
        uniqueArgs.append(recorders=[])
    ops.postprocessing.Get_Rendering.createODB(ModelName, *uniqueArgs)
