...
.. seealso::
   ...
   `Notes <http://opensees.berkeley.edu/wiki/index.php/ZeroLength_Element>`_
    ...
    if dirs:
        ...
        uniqueArgs.append(dirs)
    if rFlag=0:
        uniqueArgs.append('-doRayleigh')
        uniqueArgs.append(rFlag=0)
    if vecx:
...
def elasticBeamColumn(eleTag, eleNodes, Area, E_mod, G_mod, Jxx, Iy, Iz, transfTag, mass=None, cMass=None):
...
def ElasticTimoshenkoBeam(eleTag, eleNodes, E_mod, G_mod, Area, Iz, Jxx, Iy, Iz, Avy, Avz, transfTag, massDens=None, cMass=None):
...
def dispBeamColumn(eleTag, eleNodes, transfTag, integrationTag, mass):
    """
...
def forceBeamColumn(eleTag, eleNodes, transfTag, integrationTag, maxIter, tol, mass):
    """
