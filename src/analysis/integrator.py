...
def LoadControl(incr, numIter=1, minIncr=None, maxIncr=None):
    """
    ...
    ``minIncr`` |float|        Min stepsize the user will allow :math:`\lambda_{min}`.
                               Has value incr by default. (optional)
    ...
    if numIter:
        uniqueArgs.append(numIter)

    if minIncr:
        uniqueArgs.append(minIncr)

    if maxIncr:
        uniqueArgs.append(maxIncr)
        
    ops.integrator('LoadControl', incr, numIter, *uniqueArgs)

def DisplacementControl(nodeTag, dof, incr, numIter=1, dUmin=None, dUmax=None):
    """

    Create a DisplacementControl integrator.  In an analysis step with 
    Displacement Control we seek to determine the time step that will result in 
    a displacement increment for a particular degree-of-freedom at a node to be 
    a prescribed value.
    ...
    if dUmin:
        uniqueArgs.append(dUmin)

    if dUmax:
        uniqueArgs.append(dUmax)
            
    
    ops.integrator('DisplacementControl', nodeTag, dof, incr, *uniqueArgs)

def ParallelDisplacementControl(nodeTag, dof, incr, numIter=1, dUmin=None, dUmax=None):
    """
    ...
    ops.integrator('ParallelDisplacementControl', nodeTag, dof, incr, *uniqueArgs)

def MinUnbalDispNorm(dlambda1, Jd=1, minLambda=None, maxLambda=None, det=False):
    """
    ...
    ``Jd`` |int|               Factor relating first load increment at subsequent
                               time steps. (optional)
    ``minLambda`` |float|      Min load increment. Has value dlambda1 by default. (optional)
    ``maxLambda`` |float|      Max load increment. Has value dlambda1 by default. (optional)
    ========================   ================================================================
    ...
    if minLambda:
        uniqueArgs.append(minLambda)

    if maxLambda:
        uniqueArgs.append(maxLambda)

    if det:
        uniqueArgs.append(det)    
            
    ops.integrator('MinUnbalDispNorm', dlambda1, Jd, *uniqueArgs)

def ArcLength(s, alpha):
    """

    Create a ArcLength integrator. In an analysis step with ArcLength we seek to
    determine the time step that will result in our constraint equation being 
    satisfied.
...
def HHT(alpha, gamma, beta):
    ...
    Create a Hilber-Hughes-Taylor (HHT) integrator. This is an implicit method 
    that allows for energy dissipation and second order accuracy (which is not 
    possible with the regular Newmark object). Depending on choices of input 
...
def GeneralizedAlpha(alphaM, alphaF, gamma=None, beta=None):
    """
    Create a GeneralizedAlpha integrator. This is an implicit method that like 
    the HHT method allows for high frequency energy dissipation and second order accuracy, 
    i.e. :math:`\\Delta t^2`. Depending on choices of input parameters, 
    the method can be unconditionally stable.
    ...
    if not beta:
        gamma=(1+alphaM-alphaF)^2/4
        
    uniqueArgs.append(gamma)   
    uniqueArgs.append(beta)   
        
    ops.integrator('GeneralizedAlpha', alphaM, alphaF, *uniqueArgs)

def TRBDF2():
    """

    Create a TRBDF2 integrator. The TRBDF2 integrator is a composite scheme that 
    alternates between the Trapezoidal scheme and a 3 point backward Euler scheme. 
    It does this in an attempt to conserve energy and momentum, something Newmark 
    does not always do.
    
    As opposed to dividing the time-step in 2 as outlined in the [Bathe2007]
    we just switch alternate between the 2 integration strategies,i.e. 
    the time step in our implementation is double that described in the [Bathe2007].
    
    Bathe2007: (https://www.sciencedirect.com/science/article/abs/pii/S0045794906003099/) 
    Hints:
        untested
