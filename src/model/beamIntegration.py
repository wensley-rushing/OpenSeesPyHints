...
def Lobatto(tag, secTag, N):
    ...
    Create a Gauss-Lobatto beamIntegration object. Gauss-Lobatto integration is
    the most common approach for evaluating the response of 
    Neuenhofer and Filippou 1997 [1] because it places an integration point 
    ...
    [1] (https://ascelibrary.org/doi/10.1061/%28ASCE%290733-9445%281997%29123%3A7%28958%29) 
    """
...
def NewtonCotes(tag, secTag, N):
    ...
    """
    ops.beamIntegration('NewtonCotes', tag, secTag, N)
...
def Radau(tag, secTag, N):
   ...
   forms the basis for optimal plastic hinge integration methods.

   Places ``N`` Gauss-Radau integration points along the element with a point
   constrained to be at ndI. The location and weight of each integration point 
   are tabulated in references on numerical analysis. The force-deformation 
   response at each integration point is defined by the section. The order of 
   accuracy for Gauss-Radau integration is 2N-2.
    ...
    """
    ops.beamIntegration('Radau', tag, secTag, N)
...
def CompositeSimpson(tag, secTag, N):
    ...
    """
    ops.beamIntegration('CompositeSimpson', tag, secTag, N)
...
def UserDefined(tag, N, secTags, locs, wts):
    ...
    ::
      ...
      op.model.beamIntegration.UserDefined(1, len(secs), secs, locs, wts)
    ...
    Hint:
        It's inclear to me what "there is no accuracy for this approach
        to numerical integration" refers to, that might be a typo

    """
    ops.beamIntegration('UserDefined', tag, N, *secTags, *locs, *wts)
...
def FixedLocation(tag, N, secTags, locs):
    ...
    """
    ops.beamIntegration('FixedLocation', tag, N, *secTags, *locs)

def LowOrder(tag, N, secTags, locs, wts):
    """

   Create a LowOrder beamIntegration object.
   This option is a generalization of the :FixedLocation-BeamIntegration and 
   UserDefined-BeamIntegration approaches and is useful for moving load 
   analysis (Kidarsa, Scott and Higgins 2008 [1]). The locations of the 
   integration points are user defined, while a selected number of weights are 
   specified and the remaining weights are computed by the method of 
   ...
   Note that FixedLocation-BeamIntegration is recovered when ``Nc`` is zero.
   ...
   ========================   =============================================================
    ...
    [1] https://doi.org/10.1016/j.finel.2007.11.013
   ...
   ::
      ...
      op.model.beamIntegration.LowOrder(1, len(secs), secs, locs, wts)
   ...
   .. note::
      ...
      is recovered when ``wts`` is an empty list and
      :ref:`UserDefined-BeamIntegration`  is
...
def MidDistance(tag, N, secTags, locs):
    ...
    ::
       ...
       MidDistance(1, len(secs), secs, locs, wts)
...
def UserHinge(tag, secETag, npL, secsLTags, locsL, wtsL, npR, secsRTags, locsR, wtsR):
   ...
   ::
      ...
      op.model.beamIntegration.UserHinge(tag,secE,npL,secsL,locsL,wtsL,npR,secsR,locsR,wtsR)
...
def HingeMidpoint(tag, secI, lpI, secJ, lpJ, secE):
    ...
    The plastic hinge length at end I (J) is equal to ``lpI`` (``lpJ``) and the 
    associated force deformation response is defined by the ``secI`` (``secJ``). 
    The force deformation nresponse of the element interior is defined by the 
    ``secE``. Typically, the interior section is linear-elastic, but this is 
    not necessary.
    ...
    ::
       ...
       op.model.beamIntegration.HingeMidpoint(tag,secI,lpI,secJ,lpJ,secE)

    Hints:
        Note, this example seems incomplete.
...
def HingeRadau(tag, secI, lpI, secJ, lpJ, secE):
   ...
   Arguments and examples see HingeMidPoint-BeamIntegration.
